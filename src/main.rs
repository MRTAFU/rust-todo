...
// エラーをまとめるenumを定義します。
// actix_web::ResponseError として使うためにderiveマクロでDebugを付与している必要があります。
#[derive(Error, Debug)]
enum MyError {
...
// actix_web::ResponseError をMyErrorに実装します。
// 今回はデフォルトの実装をそのままま使いますので、新たに実装するものはありません。
impl ResponseError for MyError {}

// MyError は actix_web::ResponseErrorを実装しているので、
...
async fn index(db: web::Data<Pool<SqliteConnectionManager>>) -> Result<HttpResponse, MyError> {
  ...
  // Prepared Statement となっているSQL文を実行し、結果をTodoEntryに変換する。
